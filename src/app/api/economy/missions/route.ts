...
export async function GET(req: NextRequest) {
    const userId = "76614";
    const key = "b820c00efb2b75d05913c939001baab2"; // PRIVATE KEY FOR SERVER-SIDE
    
    // 🛡️ SECURITY: Detect User IP and Agent to pass to CPAGrip
    const ip = req.headers.get('x-forwarded-for')?.split(',')[0] || req.headers.get('x-real-ip') || '';
    const ua = req.headers.get('user-agent') || '';
    
    // 🛰️ IDENTITY SYNC: Get visitor ID for conversion logic
    const { searchParams } = new URL(req.url);
    const trackingId = searchParams.get('trackingId') || 'syndicate_guest';
    
    const url = `https://www.cpagrip.com/common/offer_feed_json.php?user_id=${userId}&key=${key}&showmobile=yes&showall=yes&tracking_id=${trackingId}${ip ? `&ip=${ip}` : ''}${ua ? `&ua=${encodeURIComponent(ua)}` : ''}`;
