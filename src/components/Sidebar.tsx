...
export default function Sidebar({ selectedProfileId, onSelectProfile, unreadCounts = {}, profiles, view = 'chats', onSetView, onOpenTopUp, isMobileUI = false }: SidebarProps) {
  ...
  const NavButton = ({ label, icon: Icon, targetView }: { label: string, icon: any, targetView: 'chats' | 'vault' | 'feed' }) => (
    <button 
        ...
        className={`flex-1 py-3 flex-col items-center gap-1 transition-all border-b-2 relative ${view === targetView ? 'border-[#ff00ff] text-white shadow-[0_10px_10px_-10px_#ff00ff]' : 'border-transparent text-white/30'}`}
    >
  ...
  useEffect(() => {
    fetchPoints();
    const interval = setInterval(fetchPoints, 60000); // 60s — event-driven for instant updates
