...
/**
 * 🛰️ SOVEREIGN CONNECTIONS HUB (NEURAL INBOX) v1.0
 * Objective: Manage long-term, high-status persona alliances via an iPhone-style terminal list.
 */
export default function ConnectionsHub({ followedIds, unreadCounts, onSelectPersona, personas }: ConnectionsHubProps) {
  const [isOpen, setIsOpen] = useState(false);

  // Filter and sort personas based on followed IDs
  const followedPersonas = personas.filter(p => followedIds.includes(p.id));
  ...
  return (
    <>
       ...
       {/* 📱 THE IPHONE-STYLE VAULT TERMINAL */}
       <AnimatePresence>
          {isOpen && (
            <motion.div 
            ...
            >
               {/* Header (Elite Messenger Vibe) */}
               <div className="p-8 border-b border-white/5 space-y-6">
                  <div className="flex items-center justify-between">
                     <h1 className="text-3xl font-syncopate font-black italic text-white tracking-tighter uppercase leading-none">Connections</h1>
               ...
               <div className="flex-1 overflow-y-auto no-scrollbar py-2">
                  {followedPersonas.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full p-12 text-center opacity-20">
                       ...
                       <p className="text-[10px] font-black uppercase tracking-[0.3em]">No Active Alliances Logged</p>
                    </div>
                  ) : (
                          ...
                          >
                             ...
                             <div className="flex-1 min-w-0">
                                ...
                                <p className="text-[10px] text-white/40 truncate italic leading-none pr-8">
                                   {p.vibe || 'Secured connection. Exclusive cache active.'}
                                </p>
                             </div>
               ...
               <div className="p-8 border-t border-white/5 bg-black/40">
                  <div className="flex items-center justify-between px-2">
                     <span className="text-[8px] font-black uppercase text-[#ffea00] tracking-widest italic">Encrypted Connection Hub</span>
                     <span className="text-[10px] text-white leading-none font-black italic">{followedPersonas.length} ALLIANCES</span>
                  </div>
               </div>
            </motion.div>
          )}
