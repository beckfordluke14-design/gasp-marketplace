...
import { Profile, initialProfiles } from '@/lib/profiles';
...
export default function FactoryPage() {
  ...
  const [grokDeckProfile, setGrokDeckProfile] = useState<Profile | null>(null);
  ...
  const handleGenerate = async () => {
    ...
    try {
        ...
        if (res.ok) {
            ...
            addLog(`[Brain] Profile '${newP.name}' Birth Successful.`);
            fetchFactoryData();
  ...
  const handleMeshSmarter = async () => {
    if (!selectedProfile || !wordBank) return;
    setIsMeshing(true);
    addLog(`[Neural Mesh] Injecting Word Bank into ${selectedProfile.name}...`);
    try {
        const res = await fetch('/api/factory', {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ persona_id: selectedProfile.id, new_words: wordBank })
        });
        if (res.ok) {
  ...
  const handleBuildContent = async (type: string) => {
    ...
    addLog(`[Studio] Building ${type} node for ${selectedProfile.name}...`);
    try {
        const res = await fetch('/api/admin/star', {
            method: 'POST',
            body: JSON.stringify({ 
                personaId: selectedProfile.id, 
  ...
  return (
    <div className="min-h-screen bg-[#020202] text-white font-outfit selection:bg-[#ff00ff] selection:text-white">
         ...
         <div className="max-w-[1600px] mx-auto h-full px-4 md:px-8 flex items-center justify-between">
            <div className="flex items-center gap-8 md:gap-16">
               <div className="flex flex-col">
                  <span className="text-xl md:text-2xl font-syncopate font-black italic tracking-tighter uppercase leading-none">Profile Factory</span>
             ...
             <div className="lg:col-span-4 space-y-8 md:y-12">
                 <div className="bg-white/5 border border-white/10 rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-10 space-y-6 md:space-y-8 shadow-2xl">
                   <div className="space-y-2">
                      <h2 className="text-lg md:text-xl font-syncopate font-bold italic tracking-tighter uppercase leading-none">Profile Architect</h2>
                      <p className="text-white/20 text-[8px] md:text-[10px] uppercase tracking-widest">Deploy standard profiles or mass-genesis.</p>
                   </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-4">
                     ...
                     >
                        {isGenerating ? 'Generating...' : 'Deploy Profile'}
                     ...
                     >
                        {isGenerating ? 'Genesis Pulse...' : 'Mass Genesis'}
              ...
              <div className="lg:col-span-5 relative">
                ...
                {studioMode && selectedProfile ? (
                   <motion.div key={selectedProfile.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-[#050505] border border-white/10 rounded-[2rem] md:rounded-[3rem] p-6 md:p-10 space-y-8 md:space-y-10 shadow-[0_0_100px_rgba(0,0,0,1)] relative overflow-hidden">
                      <div className="flex items-center gap-6 relative z-10">
                         <div className="w-20 h-20 md:w-24 md:h-24 rounded-3xl overflow-hidden relative border-2 border-[#ff00ff] shadow-[0_0_30px_rgba(255,0,255,0.3)]">
                            <Image src={selectedProfile.seed_image_url || selectedProfile.image} alt="" fill unoptimized className="object-cover" />
                         </div>
                         <div className="space-y-1">
                            <h2 className="text-2xl md:text-3xl font-syncopate font-black uppercase italic tracking-tighter leading-none">{selectedProfile.name}</h2>
                            <p className="text-[8px] md:text-[10px] font-black uppercase tracking-widest text-[#00f0ff]">{selectedProfile.city} • {selectedProfile.age}yo</p>
                         </div>
                      </div>
                   ...
                   <div className="aspect-[4/5] bg-white/5 border border-white/5 rounded-[3rem] flex flex-col items-center justify-center gap-6 text-white/10">
                      ...
                      <p className="text-[9px] font-black uppercase tracking-[0.5em] italic">Awaiting Profile</p>
                   </div>
                )}
                </AnimatePresence>
              </div>
              ...
              <div className="lg:col-span-3 space-y-8">
                <div className="flex items-center justify-between">
                   ...
                   <span className="text-[8px] font-black text-white/20 uppercase">{dbProfiles.length} nodes</span>
                </div>
                <div className="space-y-4 max-h-[80vh] overflow-y-auto no-scrollbar pb-32 font-mono">
                   {dbProfiles.map(p => (
                      <div key={p.id} onClick={() => { setSelectedProfile(p); setStudioMode(true); }} className={`flex items-center gap-4 p-4 rounded-2xl border cursor-pointer ${selectedProfile?.id === p.id ? 'bg-[#ff00ff]/10 border-[#ff00ff]/30' : 'bg-white/5 border-white/5'}`}>
                         ...
                         <button onClick={(e) => { e.stopPropagation(); handleDeleteProfile(p.id, p.name); }} className="p-2 text-red-400/40 hover:text-red-500">
                            <Trash2 size={14} />
                         </button>
                      </div>
                   ))}
      ...
      {grokDeckProfile && <LiveStudioDeck profile={grokDeckProfile} onClose={() => setGrokDeckProfile(null)} />}
    </div>
  );
}
