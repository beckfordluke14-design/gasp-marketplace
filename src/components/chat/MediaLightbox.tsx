         ...
         <div className="relative w-full h-full flex items-center justify-center p-4 overflow-hidden">
            <AnimatePresence mode="popLayout" initial={false}>
               ...
               >
                  ...
                  {/* 🛡️ REPEATING BRAND WATERMARK */}
                  <div className="absolute inset-0 pointer-events-none overflow-hidden mix-blend-overlay opacity-40">
                      {/* Repeat GASP in center */}
                      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rotate-[-45deg] opacity-[0.1] select-none">
                      ...
                      <div className="absolute inset-0 grid grid-cols-4 grid-rows-6 opacity-[0.05]">
                          {Array.from({ length: 24 }).map((_, i) => (
                              <div key={i} className="flex items-center justify-center p-4">
                                  <span className="text-[10px] font-black uppercase tracking-widest -rotate-12">GASP.FUN</span>
                              </div>
                          ))}
                      </div>
                  </div>
